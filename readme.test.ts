...
class Website {
  ...
  get routes(): Array<{ name: string, server: (request: Request) => Promise<Response> }> {
    return [
      { name: '', server: router.resource({ method: 'GET', server: this.getHome }) },
      { name: 'about', server: router.resource({ method: 'GET', server: this.getAbout }) },
      { name: 'cart', server: this.cart.fetch }
    ]
  }

  // Trivial values representing web pages in this example application
  async getHome() { return { status: 200, body: 'Home' } }
  async getAbout() { return { status: 200, body: 'About' } }
}
...
class CartWebsite {
  fetch: (request: Request) => Promise<Response> =
    router.staticDirectory(
      { name: 'view', server: router.resource({ method: 'GET', server: this.getView }) },
      { name: 'checkout', server: router.resource({ method: 'GET', server: this.getCheckout }) }
    )

  async getView() { return { status: 200, body: 'Cart' } }
  async getCheckout() { return { status: 200, body: 'Checkout' } }
}
...
class Api {
  ...
  fetch: (request: Request) => Promise<Response> =
    router.staticDirectory(
      {
        name: 'v1', server: router.staticDirectory(
          {
            name: "books-by-id",
...
class BookApi {
  fetch: (request: Request & { bookId: string }) => Promise<Response> =
    router.staticDirectory({
      name: "synopsis", server: router.resource(
...
describe("Server example", () => {
  ...
  test("root path", async () => {
    expect(await server.fetch({ method: 'GET', path: '' }))
      .toEqual({ status: 200, body: 'Home' })
  })

  test("top level path", async () => {
    expect(await server.fetch({ method: 'GET', path: 'about' }))
      .toEqual({ status: 200, body: 'About' })
  })

  test("unsupported method", async () => {
    expect(await server.fetch({ method: 'POST', path: 'about' }))
      .toEqual({ status: 405 })
  })

  test("not found", async () => {
    expect(await server.fetch({ method: 'GET', path: 'teapot' }))
      .toEqual({ status: 404 })
  })

  test("nested paths", async () => {
    expect(await server.fetch({ method: 'GET', path: 'cart/view' }))
      .toEqual({ status: 200, body: 'Cart' })
    expect(await server.fetch({ method: 'GET', path: 'cart/checkout' }))
      .toEqual({ status: 200, body: 'Checkout' })
  })

  test("dynamic directory", async () => {
    expect(await server.fetch({ method: 'GET', path: 'api/v1/books-by-id/4/synopsis' }))
      .toEqual({ status: 200, body: 'Synopsis of book 4' })
  })

  test("post", async () => {
    expect(await server.fetch({
      method: 'POST', path: 'api/v1/books-by-id/5/synopsis', body: 'abc'
    }))
